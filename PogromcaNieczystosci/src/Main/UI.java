...
public class UI {
    ...
    BufferedImage symbol_plastik, symbol_szklo, symbol_papier, symbol_mieszane; // pokazywane w lewym górnym rogu obrazik przy liczniku załadunku są "wyciągane" z klas pojemników
    public boolean czy_powiadomienie = false; // zmienna do sprawdzenia czy należy poinformować gracza, że musi rozładować śmieciarkę
    public String powiadomienie ="";
    public int czas_powiadomienia = 0;
    public boolean porażka_pomieszanie = false;
    public int wybrana_opcja = 0; // zmienna do rysowania strzałek w menu i monitorowania na którym aktualnie wyborze znajduje się gracz po to, aby po wciśnięciu "ENTER" została wybrana poprawna oopcja
    ...
    private int klatki = 60;  // zmienna używana do odliczania czasu
    public int któryEkranMenu = 0; // zmienna, o którą opiera się mechanizm przechodzenia między menu, a wyborem czasu na poziom


    private boolean odliczanie = false;

    public UI(GamePanel gp){ // ustawienie zdefiniowanych czcionek i pobranie obrazów z klas pojemników
        this.gp = gp;
    ...
    public void draw(Graphics2D g2){  // rysowanie wyznaczonych ekranów w zależności od stanu gry
        this.g2=g2;
    ...
    public void rysujMenu(){
        if(któryEkranMenu == 0){  // ekran startowy
            ...
            y += gp.rozmiar_pola*4;
            g2.drawString(text, x, y);
            if (wybrana_opcja == 0){  // warunek używający zmienną opisaną na początku całej klasy
        ...
        else if (któryEkranMenu == 1){ // ekran wyboru czasu na poziom
            g2.setColor(Color.gray);
            g2.fillRect(0,0, gp.szerokosc, gp.dlugosc);
            g2.setFont(calibri_36);
            g2.setColor(Color.black);
    ...
    public void rysujInstrukcje(){  // ekran po wybraniu w menu opcji "Instrukcja"
        g2.setColor(Color.gray);
        g2.fillRect(0, 0, gp.szerokosc, gp.dlugosc);
        ...
        for (String line : lines){ // zastosowanie pisania tekstu innym sposobem, ponieważ bez niego, nie działało zakończenie linii
    ...
    public void licznik_zebranych_śmieci(){
        if(gp.StanGry == gp.StanGranie){  // włącznienie czasu równocześnie z rozpoczęciem gry
            updateLicznikCzasu();
        }
        if (gp.StanGry != gp.StanMenu || gp.StanGry != gp.StanKoniec){ // warunek pojawienia się licznika śmieci na śmieciarce
            g2.setFont(calibri_36);
            g2.setColor(Color.white);
            g2.drawImage(symbol_mieszane, gp.rozmiar_pola/4, -10, gp.rozmiar_pola, gp.rozmiar_pola, null);  // rysowanie obrazków pojemników wyciągniętych z klas pojemników
            ...
            if(czy_powiadomienie == true){
                ...
                czas_powiadomienia++; // rozpoczęcia liczenia czasu pojawienia się powiadomienia, w mechanice kolizji ustawiono, aby czas był liczony od momentu przerwania kontaktu z pojemnikiem

                if(czas_powiadomienia > 240){  // ustawienie czasu wyświetlania się powiadomienia - 240:6 => 4 sekundy
    ...
    public void rysujPauze(){ //powiadomienie wyświetlające się w trakcie pauzy
    ...
    public int Srodek_ośX(String text){  // sprowadzenie wyśrodkowania tekstu do metody
    ...
    public void updateLicznikCzasu(){  // dekrementacja czasu na poziom i wywołanie porażki gdy czas wyniesie zero
        if (odliczanie) {
            czasNaPoziom--;
        }
        if(czasNaPoziom <=0){
    ...
    public void rysujLicznikCzasu(){  // rysowanie licznika czasu w prawym górnym rogu ekranu jeżeli program jest w stanie gry
    ...
    public void koniecGryPomieszanie(){  // powiadomienie wyświetlające się, gdy gracz złamie regułę mieszania śmieci
    ...
    public void koniecGryCzas(){ //powiadomienie wyświetlające się, gdy skończy się czas na poziom
    ...
    public void koniecGry(){             // ekran po wygraniu gry

        Klawiatura.czy_grano = false;    // reset ustawień na wypadek gdyby gracz chciał spróbować ponownie
